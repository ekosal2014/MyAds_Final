(function($){
	...
	$.fn.carousel = function( method )
	{
		...
		function initialize($this)
		{
			...
			var _margin;
			$(".item_related",content).each(function(index, element) {
			...
			if($(".item_related",content).length <= 1)
			{
		...
		function _resizeHandlerCarousel ()
		{
			if(plugin_settings.responsive.minWidth)
			{
				...
				else
				{
					...
					if(plugin_settings.resizeItem) {
						if($(".item_related",content).attr('style'))
						{
							$(".item_related",content).each(function(index, element) {
		...
		function _moveSlide (arg_)
		{
			...
			var widthItem = $('.item_related',content).outerWidth(true);//tamanho do item
